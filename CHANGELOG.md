...
- Presence follows the active canvas, composition and terminal path in shared titlebars instead of separate roster overlays. (#216, #246)
- Terminal icons and symbols use a bundled Nerd Font subset rather than relying on fonts installed on your device. (#217, #246)
- Each terminal has independent font-size controls, saved on your device without changing other terminals or collaborators' views. (#224, #246)
...
- The protocol now speaks isolation (ADR 0016 stage 1+2): the two frame pairs an installed plugin's server process and browser Worker talk to the engine over, the closed component vocabulary its panels render with instead of touching the DOM, the install artifact (`<id>.manifold-plugin.json`, pinned by hash), the `install` block a roster row carries with the capabilities its installer granted, the `unavailable` refusal for an isolate that crashed or did not answer, and the isolate lifecycle states — all published under `isolateContract` at `GET /api/protocol`, so an out-of-tree plugin author reads the whole target from one document. (#151, #223)
- The server can run an installed plugin's server half in its own process (ADR 0016 stage 1): a supervisor spawns the bundle's `server.js` with a minimal environment, hands its doors to the roster exactly as first-party ones, serves the plugin's storage, authority and placement calls back from the dispatching caller's own context, bounds every round trip by the dispatch deadline, and applies the crash budget — three unasked-for exits in five minutes and the row reads `isolate_crashed` with its doors answering `unavailable` until it is toggled — while an idle child is evicted after ten minutes and spawned again on the next dispatch. (#151, #231)
- A plugin can be installed from a bundle (ADR 0016 stage 2): `engine.plugins.install { source, sha256, grant?, replace? }` and `engine.plugins.uninstall { id }` are root-only doors on the engine's row, refusing by a named class (`hash_mismatch`, `already_installed`, `namespace_reserved`, `still_enabled`, …). The bundle's bytes are pinned by hash before anything is written and re-verified at every boot, a tampered one is refused on its roster row rather than loaded, the installer's grant (declared caps minus `*`, `tokens:mint`, `plugins:manage` unless widened) is published on the row and enforced at the door before the caller's own caps, `GET /api/plugins/:id/web.js` serves an installed plugin's web half, and the plugin manager's Installed band exposes the form and the list over those doors. (#152, #231)
- `@manifold/plugin-kit`, the authoring kit for an isolated (out-of-tree) plugin: `defineServerPlugin` and `defineWebPlugin` are the two guest runtimes — the child's end of the ipc channel and the worker's end of `postMessage`, serving the plugin its ctx and host over `call` frames and grading only the two rungs the child may — `ui` is one builder per kind of the closed component vocabulary, and `bun run --cwd packages/plugin-kit pack <dir> --out <file>` turns a directory into the self-contained `<id>.manifold-plugin.json` artifact and prints the sha256 an installer pins. `docs/PLUGINS.md` §9 is the second authoring target written down, including what an isolated plugin does not get. (#151, #230)
- An installed plugin's panels now render in the browser (ADR 0016 stage 1, web half): the shell runs each installed plugin's web half in its own dedicated Worker, fetched with your credentials so the plugin itself never holds them, and paints whatever the plugin describes through the engine's closed component vocabulary — headings, text, code, badges, buttons, selects, inputs, toggles, lists — in the shell's own colours, with every button naming the door it opens. A plugin that breaks shows its fault in its tile and touches nothing else. The whole path is now proven end to end on every gate run: the kit's reference plugin is packed, installed through the door, dispatched from its own process, seated as a panel, painted from its Worker in a real browser and bumped by a click (`verify:axioms` R11), and the server half's install, restart, tamper and uninstall contracts are held by an e2e suite. (#151, #237)
- Plugin manifests can declare repository, homepage and changelog URLs. (#238, #241)

### Changed

- Development builds can carry a distinct app title and orange icon, keeping dev visibly separate from production. (#221, #246)
- Canvas containers and composition tiles use the same readable titlebar and control layout. (#220, #246)
- Windows are dragged by their titlebars instead of separate grip icons, leaving their content available for interaction. (#222, #246)
- Embedded compositions render at native scale, with rounded outer frames and flush, square internal seams. (#225, #246)
- Publishing a release no longer deploys production. Production promotion is a separate, explicit workflow selecting a published release tag. (#244, #246)
- `ctx.storage` is promise-returning for every plugin, first-party included, as ADR 0016 §4 ratified: every read and write is awaited, a refused key or value rejects instead of throwing, migrations may be `async`, and the server binds its socket only after the plugin host has booted and run every pending migration. (#187, #215)
...
- The plugin manager is one list instead of two tabs: three collapsible bands (Installed, grouped by publisher; Built-in; Engine, folded by default) with counts and a "17 of 19 on" summary, a plugin family (`atyrode.code` and the parts that require it) as one row with a chevron and its parts nested under it — the parent's switch turns the family off and on, a part's switch says "atyrode.code is off" while it cannot move — status and permissions as plain-words chips on every row ("Crashed", "Refused: its bundle no longer matches its hash", "granted 1 of 2 declared"), sort by name / status / recently changed / permissions, filter chips (On, Off, Needs attention, Installed, Built-in), a search that also reads door names, and a detail sheet beside the list with status, permissions and their meanings, doors, contributions, family, relations as links, settings, the manifest's repository / homepage / changelog links, and a danger zone. Install lives under the Installed band's heading, with the withheld capabilities shown as chips to grant. The "Needs / Needed by" lines and the lock glyph are gone from rows. (#239, #243)

### Fixed

- Terminal content meets its titlebar and frame without empty chrome gaps or mismatched corners. (#218, #246)
- Rearranging tiles previews the vacated source correctly as a carry leaves and re-enters, in fullscreen and embedded compositions. (#219, #246)
- The plugin manager explains when retained data must be purged before uninstalling a plugin. (#245, #246)
...
- The verification gates ask the kernel for a free loopback port — for the throwaway server and for every Chromium they launch — instead of picking one blindly inside the ephemeral range, so a sibling gate's outbound connection can no longer turn a docs-only run red with `Is port N in use?`. (#198, #214)
- Uninstalling a plugin no longer strands its data or its switch: `engine.plugins.uninstall` refuses `storage_retained: <n> keys; purge first or pass purge: true` while the plugin's storage holds anything, `purge: true` purges first through the purge door's own path and event, and a reinstall of the same id comes back ON like a first install instead of inheriting the OFF it was switched to for the uninstall. An installed bundle that fails re-verification at boot now keeps its doors on the roster — recorded on the install row when it was admitted, never read from the file — and each answers `unavailable` naming the refusal (`bundle failed verification at boot: hash_mismatch`), traced like every refusal, instead of the untraced `unknown_action`. (#233, #241)
