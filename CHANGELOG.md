...
- Arrange mode stopped drawing handles. A row you can move is shown by its tint, not by a grip icon in its corner: the whole row is the grab surface, so a corner icon was pointing at a fraction of what was live, and a rail of them read as clutter instead of as an offer. Rows still announce themselves to a screen reader and still move with the arrow keys. (#94, #PR)
