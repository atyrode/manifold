...
- Traceability is now constitutional rather than a feature: axiom A6 says every exercise of authority leaves a trace, it names its own exemptions (live presence, terminal keystrokes, in-progress document edits — each traced at the point it commits, where it has one), and a new gate check dispatches every registered action against a real server and fails RED if any of them leaves no record. A future feature cannot quietly opt out of being auditable. (#93, #PR)
