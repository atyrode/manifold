...
describe("machine-channel compatibility (AGENTS.md invariant 10)", () => {
  test("v21 ADDS to the acceptance set, because the agent wire still did not move", () => {
    /*
      ...
      is precisely the machine token) and v20 -> v21 (the carry aim's layout revision and the
      ...
      losing and renaming nothing; an agent never sees a principal, a session frame or a
      browser's throttled timers, and no credential an enrolled spoke holds changed meaning
      at v20. So the invariant's first clause applies verbatim — a bump that leaves the agent
