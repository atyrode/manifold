...
import { attachWorkerInput, JsonFrameReader, WorkerError } from "../src/worker.ts";
...
test("typed stdin frames preserve fragmented UTF-8 and process coalesced frames without accumulating the chunk", () => {
  const frames: string[] = [];
  const reader = new JsonFrameReader({ parse: parseString, receive: (frame) => frames.push(frame), maxFrameBytes: 8 });
...
test("frame ceilings include the newline and reject a fragmented oversized line before JSON parsing", () => {
  ...
  const exact = new JsonFrameReader({ parse: (value) => value, receive: (value) => frames.push(value), maxFrameBytes: 6 });
  exact.push(Buffer.from('"abc"\n'));
  expect(frames).toEqual(["abc"]);
  const overflow = new JsonFrameReader({ parse: (value) => value, receive: (value) => frames.push(value), maxFrameBytes: 6 });
...
test("invalid UTF-8, invalid domain frames and truncated EOF fail without leaking payloads", () => {
  for (const bytes of [Buffer.from([34, 0xc0, 0xaf, 34, 10]), Buffer.from("{\"private\":\"secret\"}\n"), Buffer.from("\n")]) {
    const reader = new JsonFrameReader({ parse: parseString, receive: () => { throw new Error("must not deliver"); } });
    expect(() => reader.push(bytes)).toThrow("worker_input_invalid");
  }
  const partial = new JsonFrameReader({ parse: parseString, receive: () => { throw new Error("must not deliver"); } });
...
test("stdin EOF closes exactly once after delivering complete typed frames", async () => {
  ...
  let closes = 0;
  const detach = attachWorkerInput({ input, parse: parseString, receive: (frame) => frames.push(frame), onClose: (error) => {
    closes++;
    ended.resolve(error);
  } });
  try {
...
test("stdin cancellation detaches promptly and does not deliver subsequent frames", () => {
  ...
  const frames: string[] = [];
  const errors: string[] = [];
  const detach = attachWorkerInput({ input, signal: controller.signal, parse: parseString,
    receive: (frame) => frames.push(frame), onClose: (error) => errors.push(error.code) });
  try {
    input.write('"start"\n');
...
test("stdin EOF with a partial control frame is a refusal, not a successful final frame", async () => {
  const input = new PassThrough();
  const ended = Promise.withResolvers<WorkerError>();
  const detach = attachWorkerInput({ input, parse: parseString, receive: () => { throw new Error("must not deliver"); }, onClose: ended.resolve });
  try {
    input.end('"partial"');
