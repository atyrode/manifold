...
const DEFS: readonly WebPluginDef[] = [
  ...
  { id: "core.draw", elements: { draw: DrawNode }, tools: { draw: { title: "Ink" } } },
...
describe("buildWebComposition", () => {
  test("registrations attach to declared names, keyed the way each kind is addressed", () => {
    ...
    // The MANIFEST owns the published title; a registration's own title is a hint carried
    // inside `contribution`, never a way to rename what the roster declared.
    expect(composition.tools).toEqual([
      {
        id: "draw",
        plugin: "core.draw",
        title: "Draw",
        contribution: { title: "Ink" },
        enabled: true,
      },
    ]);
  ...
  test("a declared name with no registration is PRESENT with a null component", () => {
    ...
    expect(composition.tools[0]?.contribution).toBeNull();
