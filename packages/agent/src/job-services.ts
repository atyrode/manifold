...
import {
  SERVICE_FRAME_BYTES, ServiceBindingSchema, ServiceCallSchema, ServicePolicySchema,
  type ServiceBinding, type ServiceCall, type ServiceInput, type ServiceOperationPolicy,
  type ServicePolicy, type ServiceRefusal, type ServiceReply,
...
export interface JobServiceRunner {
  call(request: ServiceCall, binding: ServiceBinding, authorize: AuthorizeServiceCall, signal?: AbortSignal): Promise<ServiceReply>;
  /** Abort active requests; descriptors supplied by the owner remain borrowed. */
  close(): void;
}
class ServiceFailure extends Error {
  constructor(readonly refusal: ServiceRefusal) { super(refusal); }
}
...
export function heldServiceCredentialResolver(descriptors: ReadonlyMap<string, number>): ResolveServiceCredential {
  ...
  return async (ref, signal) => {
    ...
    if (!before.isFile() || before.uid !== BigInt(process.getuid?.() ?? -1) ||
      (before.mode & 0o077n) !== 0n || before.nlink !== 1n || before.size < 1n || before.size > 16384n)
    ...
    try {
      ...
      if (offset !== Number(before.size) || before.size !== after.size || before.mtimeNs !== after.mtimeNs ||
        before.ctimeNs !== after.ctimeNs || before.mode !== after.mode || before.nlink !== after.nlink)
        throw new ServiceFailure("service_credential_unavailable");
      // A single terminal line ending is file framing, not part of an HTTP credential.
      return new TextDecoder("utf-8", { fatal: true }).decode(bytes.subarray(0, offset)).replace(/\r?\n$/, "");
...
function prepareRequest(operation: ServiceOperationPolicy, input: ServiceInput, origin: string): { url: URL; body?: string } {
  ...
  for (const [key, field] of Object.entries(operation.input)) {
    ...
    if ((field.type === "string" && (typeof value !== "string" || Buffer.byteLength(value) > field.maxBytes || (field.enum && !field.enum.includes(value)))) ||
      (field.type === "number" && (typeof value !== "number" || value < field.min || value > field.max || (field.integer && !Number.isSafeInteger(value)))) ||
  ...
  if (url.origin !== origin || url.pathname !== path) throw new ServiceFailure("service_input_invalid");
  ...
  for (const field of operation.body) {
    const value = "input" in field.value ? input[field.value.input] : field.value.literal;
    ...
    for (let i = 0; i < field.path.length - 1; i++) {
      ...
      if (child === null || typeof child !== "object") throw new ServiceFailure("service_input_invalid");
      node = child;
    }
  ...
  if (Buffer.byteLength(url.pathname + url.search) + Buffer.byteLength(body ?? "") > operation.maxRequestBytes)
    throw new ServiceFailure("service_input_invalid");
  return { url, body };
}

async function transport(url: URL, operation: ServiceOperationPolicy, body: string | undefined, headers: Record<string, string>, signal: AbortSignal): Promise<Buffer> {
  signal.throwIfAborted();
  let response: IncomingMessage | undefined;
  try {
    const { promise, resolve, reject } = Promise.withResolvers<IncomingMessage>();
    const req = (url.protocol === "https:" ? httpsRequest : httpRequest)(url, {
      method: operation.method, agent: false, signal, maxHeaderSize: 16384,
      headers, ...(url.protocol === "https:" ? { rejectUnauthorized: true } : {}),
    }, resolve);
    ...
    if (length !== undefined && (!/^\d+$/.test(length) || Number(length) > operation.maxResponseBytes))
      throw new ServiceFailure("service_response_limit");
    if (operation.response.kind !== "bytes" && !/^application\/(?:[a-z0-9!#$&^_.+-]+\+)?json(?:\s*;|$)/i.test(response.headers["content-type"] ?? ""))
...
function projectionTree(fields: string[][]): Projection {
  ...
  for (const path of fields) {
    ...
    for (const part of path) {
      ...
      if (!child) { child = { leaf: false, children: new Map() }; node.children.set(part, child); }
      node = child;
    }
    node.leaf = true;
  }
...
function project(value: unknown, node: Projection, maxArrayItems: number, budget: { nodes: number }): unknown {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new ServiceFailure("service_response_invalid");
  const result: Record<string, unknown> = Object.create(null);
  for (const [key, child] of node.children) {
    if (Object.hasOwn(value, key)) result[key] = project(Reflect.get(value, key), child, maxArrayItems, budget);
  }
  return result;
}
function inspectJson(value: unknown, secret: string | undefined): void {
  ...
  while (stack.length) {
    ...
    if (typeof item === "string") {
      if (secret && item.includes(secret)) throw new ServiceFailure("service_response_invalid");
      ...
      for (const [key, child] of entries) {
        if (secret && key.includes(secret)) throw new ServiceFailure("service_response_invalid");
...
export function createJobServiceRunner(options: { policies: readonly ServicePolicy[]; resolveCredential?: ResolveServiceCredential }): JobServiceRunner {
  const policies = new Map<string, { policy: ServicePolicy; active: number; projections: Map<string, Projection> }>();
  ...
  return {
    ...
    async call(raw, rawBinding, authorize, callerSignal) {
      ...
      const refusal = (code: ServiceRefusal): ServiceReply => ({ type: "service_result", requestId, ok: false, refusal: code });
      ...
      try {
        ...
        const authority = Object.freeze({ serviceId: request.serviceId, revision: entry.policy.revision, operationId: request.operationId, input: Object.freeze(request.input) });
        try {
          if (await abortable(authorize(authority, controller.signal), controller.signal) !== true)
            throw new Error("denied");
        } catch { throw new ServiceFailure("service_unauthorized"); }
        controller.signal.throwIfAborted();
        const headers: Record<string, string> = { "accept-encoding": "identity", accept: operation.response.kind === "bytes" ? "application/octet-stream" : "application/json" };
        ...
        if (credential) {
          try {
            ...
            secret = await abortable(resolveCredential(credential.ref, controller.signal), controller.signal);
          ...
          } catch { throw new ServiceFailure("service_credential_unavailable"); }
        }
        controller.signal.throwIfAborted();
        // Credential resolution may await rotation. Recheck current consent immediately before I/O.
        if (credential) {
          try {
            if (await abortable(authorize(authority, controller.signal), controller.signal) !== true) throw new Error("denied");
          } catch { throw new ServiceFailure("service_unauthorized"); }
        }
        const bytes = await transport(prepared.url, operation, prepared.body, headers, controller.signal);
        let result: unknown;
        if (operation.response.kind === "bytes") {
          if (secret && bytes.includes(Buffer.from(secret))) throw new ServiceFailure("service_response_invalid");
          result = { encoding: "base64", data: bytes.toString("base64") };
        } else {
          let json: unknown;
          try { json = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)); }
          catch { throw new ServiceFailure("service_response_invalid"); }
          result = operation.response.kind === "projected-json"
            ? project(json, entry.projections.get(request.operationId)!, operation.response.maxArrayItems, { nodes: 65536 }) : json;
          inspectJson(result, secret);
        }
        const serialized = JSON.stringify(result);
        if (Buffer.byteLength(serialized) > operation.maxResultBytes) throw new ServiceFailure("service_response_limit");
        controller.signal.throwIfAborted();
        // Result is JSON from bounded parsing/projection, never upstream headers or exception text.
        const reply = { type: "service_result" as const, requestId, ok: true as const, result: result as Extract<ServiceReply, { ok: true }>["result"] };
        if (Buffer.byteLength(JSON.stringify(reply)) + 1 > SERVICE_FRAME_BYTES) throw new ServiceFailure("service_response_limit");
        return reply;
      } catch (error) {
        if (controller.signal.aborted) return refusal(closed ? "service_closed" : timedOut ? "service_timeout" : "service_cancelled");
        return refusal(error instanceof ServiceFailure ? error.refusal : "service_upstream_refused");
      } finally {
        secret = undefined;
        clearTimeout(timer);
