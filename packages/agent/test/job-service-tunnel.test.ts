...
test("real HTTP upload EOF preserves a gated streaming response through paired tunnels", async () => {
  const tunnels = pair();
  ...
  const requestEof = Promise.withResolvers<void>();
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      // This is TCP EOF, not just HTTP Content-Length completion. The response
      // deliberately starts afterwards, and remains open after its first bytes.
      await requestEof.promise;
  ...
  server.on("connection", (socket) => {
    serverSocket = socket;
    socket.once("end", requestEof.resolve);
  });
  ...
  try {
    ...
    tunnels.left.stream.end(Buffer.concat([headers, upload]));
    ...
    expect(tunnels.left.stream.writableFinished).toBe(true);
    expect(tunnels.left.stream.destroyed).toBe(false);
...
test.each(["abort", "peer close"] as const)(
  ...
  async (cause) => {
    ...
    try {
      ...
      expect([...tunnels.leftFrames, ...tunnels.rightFrames].filter((frame) => frame.type === "close"))
        .toHaveLength(1);
      ...
      expect(tunnels.errors.every((error) => !error.message.includes("private-owner-token"))).toBe(true);
    } finally {
      tunnels.close();
    }
  },
...
test.each(["refused", "throws"] as const)("a %s transport fails the active write safely", async (mode) => {
...
const invalidFrames: [string, unknown][] = [
  ...
  ["extra field", { type: "data", channelId, sequence: 0, data: "AA==", secret: "private-peer-detail" }],
  ...
  ["unsafe sequence", { type: "data", channelId, sequence: Number.MAX_SAFE_INTEGER + 1, data: "AA==" }],
  ["empty data", { type: "data", channelId, sequence: 0, data: "" }],
  ["oversize data", { type: "data", channelId, sequence: 0, data: Buffer.alloc(FRAME_BYTES + 1).toString("base64") }],
...
test.each(invalidFrames)("rejects %s without releasing bytes or leaking peer content", async (_name, frame) => {
  ...
  expect(sent.filter((outbound) => outbound.type === "close")).toEqual([{ type: "close", channelId }]);
});
...
test.each(["ack", "end", "data"] as const)("rejects a replayed %s after valid traffic", async (type) => {
  const tunnels = pair();
  try {
    const written = Promise.withResolvers<void>();
    ...
    if (type === "data") {
      ...
      frame = { type: "data", channelId, sequence: 0, data: Buffer.from("request").toString("base64") };
    } else if (type === "end") {
      target = tunnels.right;
      tunnels.left.stream.end();
