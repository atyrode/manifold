      ...
      while ((newline = pending.indexOf("\n")) !== -1) {
        ...
        if (command.type === "status_request") {
          frame = {
            ...
            build: "retirement-fixture",
            pid: process.pid,
            draining: state.draining,
            transportAttached: state.transportRunning,
            terminals: [],
          };
          ...
          if (!state.draining || state.transportRunning || scenario === "jobs") {
            frame = {
              ...
              reason: scenario === "jobs" ? "jobs_retained" : "not_draining",
              terminalIds: [],
            };
          } else {
...
`,
  ...
  writeFileSync(
    join(directory, "busctl"),
    `#!${process.execPath}
const args = process.argv.slice(2);
if (args[0] !== "--user" || args[1] !== "--json=short") process.exit(1);
if (args[2] === "call" && args[6] === "GetUnit") {
  const unit = args[8];
  if (unit !== "old-owner.service" && unit !== "old-transport.service") process.exit(1);
  console.log(JSON.stringify({ type: "o", data: ["/org/freedesktop/systemd1/unit/" + (unit === "old-owner.service" ? "owner" : "transport")] }));
} else if (args[2] === "get-property") {
  const property = args[6];
  if (property !== "ExecStop" && property !== "ExecStopPost") process.exit(1);
  const unproved = args[4].endsWith("/transport") && property === "ExecStopPost" && process.env.FIXTURE_SCENARIO === "transport-stop-command";
  console.log(JSON.stringify({ type: "a(sasbttttuii)", data: unproved ? [["unproved-stop-command"]] : [] }));
} else process.exit(1);
`,
    { mode: 0o700 },
  );
...
`,
      "retirement-fixture",
      join(import.meta.dir, "../../../infra/previews/retire-spoke.sh"),
      "--container",
      "fixture-hub",
      "--machine-id",
      machineId,
      ...
      "old-owner.service",
      "--transport-unit",
      "old-transport.service",
      "--transport-package",
      `/nix/store/${"a".repeat(32)}-manifold-agent-fixture`,
      "--socket",
      socketPath,
      "--runtime-dir",
      directory,
    ],
    {
      env: {
        ...
        FIXTURE_HUB: hub.url.origin,
        FIXTURE_KEY_PATH: ownerKeyPath,
