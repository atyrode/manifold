...
function channel(
  callbacks: Pick<ConstructorParameters<typeof JobContext>[1], "serviceReady"> = {},
) {
  ...
  const context = new JobContext("runtime-child", {
    invoke: () => {
      throw new Error("readiness must not invoke a child");
    },
    command: async () => {
      throw new Error("readiness must not issue a job command");
    },
  ...
  return {
    ...
    send(value: unknown) {
      child.write(`${JSON.stringify(value)}\n`);
    },
  ...
  test("an ordinary job cannot claim runtime-service authority; missing admission callback refuses", async () => {
    ...
    try {
      ...
      expect(await c.next()).toEqual({
        ...
        requestId: "ready-2",
        ok: false,
        refusal: "service_ready_duplicate",
      });
