...
export function migrateToDurableAgents(db: Database): void {
  const counts = db.query<{
    ...
    (SELECT COUNT(*) FROM terminals) AS terminals`).get()!;
  const invalidLineage = db.query<{ invalid: number }, []>(`SELECT COUNT(*) AS invalid
    ...
    WHERE p.id IS NULL OR p.kind<>'agent' OR root.id IS NULL
      ...
      OR NOT EXISTS(SELECT 1 FROM agent_run_policy_snapshots s
        WHERE s.run_id=r.id AND s.revision=r.policy_revision)`).get()!;
...
`);
  const rows = db.query<LegacyAgent, []>(`SELECT r.principal_id,p.name,r.authorized_by_principal_id
    FROM agent_runs r JOIN principals p ON p.id=r.principal_id ORDER BY r.principal_id`).all();
  const reserved = new Set(rows.map((row) => JSON.stringify([row.authorized_by_principal_id, row.name])));
  ...
  db.exec(`
INSERT INTO agent_runs_v37 SELECT r.*,r.principal_id,NULL,NULL,'unknown' FROM agent_runs r;
...
`);
  const migratedCounts = db.query<typeof counts & { agents: number }, []>(`SELECT
    ...
    (SELECT COUNT(*) FROM job_schedule_occurrences) AS occurrences,
    (SELECT COUNT(*) FROM terminals) AS terminals`).get()!;
  if (migratedCounts.agents !== counts.runs ||
      (Object.keys(counts) as Array<keyof typeof counts>).some((key) => counts[key] !== migratedCounts[key])) {
  ...
  const columns = db.query<{ name: string }, []>("PRAGMA table_info(agent_runs)").all()
    .map(({ name }) => `"${name.replaceAll('"', '""')}"`).join(",");
  const changedRuns = db.query<{ invalid: number }, []>(`SELECT COUNT(*) AS invalid FROM (
    SELECT ${columns} FROM agent_runs EXCEPT SELECT ${columns} FROM agent_runs_v37
  )`).get()!;
  const invalidBackfill = db.query<{ invalid: number }, []>(`SELECT
    ...
    + (SELECT COUNT(*) FROM terminals t WHERE t.run_id IS NOT
        (SELECT id FROM agent_runs r WHERE r.principal_id=t.created_by)) AS invalid`).get()!;
