...
import { FakeClock, FakeRuntime, FakeSocket, testPluginHost, testStore, testTileTrees } from "./helpers.ts";
...
const machine: MachineHalf = {
  artifacts: {
    "linux-x64": {
      url: "https://example.invalid/harness", sha256: hash, entrySha256: hash,
      format: "raw", entry: ["harness"], maxBytes: 4096, maxExpandedBytes: 4096, maxMembers: 1,
    },
  },
  operations: {
    [operationId]: {
      argv: [], input: {}, runtimeTools: [], locations: [], outputs: [], network: "none",
      limits: { timeoutMs: 60000, memoryBytes: 1048576, processes: 2, outputBytes: 65536 }, stdin: true,
    },
  },
  locations: {},
};
...
function result(outcome: ActionOutcome): unknown {
  if (!outcome.ok) throw new Error(`action refused: ${outcome.denial.rule}: ${outcome.denial.message}`);
...
async function fixture() {
  ...
  store.createContainer({ id: containerId, name: "Run", createdAt: runtime.now(), discipline: "composition" });
  const descriptor: TerminalRuntime = {
    machineId, pluginId, operationId, installationRevision: "r1", artifactSha256: hash,
    resourceBindingDigest: createHash("sha256").update("null").digest("hex"), input: {},
  };
  const definition: ServerPluginDef = {
    manifest: {
      id: pluginId, version: "1.0.0", title: "Test harness", description: "Binding boundary fixture",
      capabilities: ["machines:run", "jobs:read", "jobs:input"], machine,
      contributes: {
        panels: [], sections: [], elements: [], tools: [], events: [],
        harness: { id: "test-harness", title: "Test harness", profileSchema: { type: "object" }, sessionRef: "typed" },
      },
    },
    actions: [], handlers: {},
    harness: {
      ...
      async launch(ctx, run) {
        ...
        return { runtime: descriptor, session: { harness: "test-harness", machineId, sessionId: run.id }, reviewDigest: hash };
      },
      async sessions(ctx) {
      ...
      async send(ctx, run, input) {
        ...
        await ctx.jobs.input({ node, requestId: ctx.newId(), seq: job.nextInputSeq, data: Buffer.from(input).toString("base64"), eof: false });
      },
    },
  };
  ...
  const host = await testPluginHost(store, auth, rooms, broker, runtime, { settingsPlugins: [definition] });
  const service = new JobService(store, auth, runtime);
  host.setJobs(service);
  broker.setJobs(service);
  service.install(root, { machineId, pluginId, installationRevision: "r1", artifactSha256: hash, machine });
  service.consent(root, {
    machineId, pluginId, installationRevision: "r1", artifactSha256: hash,
    node: `manifold://machine/${machineId}/operation/${operationId}`, cap: "machines:run", enabled: true,
  });
  const commands: JobCommand[] = [];
  const sent: ServerToAgentMessage[] = [];
  const channel = {
    machineId, protocolVersion: 32, terminalHostId: "terminal-host", terminalExecution: "governed" as const,
  ...
  const owner: JobOwner = {
    protocolVersion: JOB_OWNER_PROTOCOL_VERSION, ownerId: "native-owner", terminalHostId: "terminal-host",
    publicKey: pair.publicKey.export({ type: "spki", format: "pem" }).toString(), generation: 1,
  ...
  const connect = (protocolVersion: number, ownerProtocolVersion: number) => {
    ...
    if (ownerProtocolVersion === JOB_OWNER_PROTOCOL_VERSION) {
      ...
      const proof = { nonce: challenge.nonce, serverEpoch: challenge.serverEpoch, machineId, owner: advertised };
      service.event(channel, { type: "owner_proof", ...proof, signature: sign(null, Buffer.from(canonicalJobJson(proof)), pair.privateKey).toString("base64") });
      service.event(channel, { type: "installed", pluginId, installationRevision: "r1", artifactSha256: hash });
    }
    broker.setMachineOnline(channel);
  };
  connect(32, JOB_OWNER_PROTOCOL_VERSION);
  const grant = {
    caps: ["containers:read"] as const, targets: ["manifold://"], reach: "subtree" as const,
    maxRunLifetimeMs: 60000, delegation: { maxDepth: 0, maxDescendants: 0 }, expiresAt: runtime.now() + 600000,
  };
  const registered = auth.registerAgent({ name: "Harness agent", purpose: "Inspect the launch boundary", harness: "test-harness", grant: { ...grant, caps: [...grant.caps] }, context: { profile: { label: "reviewed" } } }, root);
  const create = async () => CreateRunResultSchema.parse(result(await host.dispatch(root, "core.access.createRun", { agentId: registered.agent.agentId, target: { machineId, containerId } })));
  const launch = async (runId: string) => LaunchRunResultSchema.parse(result(await host.dispatch(root, "core.access.launchRun", { runId })));
  const open = async (value: TerminalRuntime, actor: AuthContext = root) => {
    ...
    const request = { elementId: runtime.newId(), machineId, cols: 80, rows: 24, placement: "tile" as const, runtime: value };
    const admission = z.strictObject({ traceId: z.number() }).parse(result(await host.dispatch(actor, "core.terminals.open", { ...request, containerId })));
    broker.open(peer, { type: "terminal_open", ...request }, admission.traceId);
    return socket;
  };
  return { auth, root, host, store, clock, registered, descriptor, definition, create, launch, open, sent, connect,
    duplicate: () => testPluginHost(store, auth, rooms, broker, runtime, {
      settingsPlugins: [definition, { ...definition, manifest: { ...definition.manifest, id: "test.other-harness" } }],
    }),
    close() { host.close(); store.close(); },
  };
}
...
test("browser descriptors bind distinct runs without returning or journaling their credential", async () => {
  ...
  try {
    ...
    const other = f.auth.mintToken({ principal: { name: "Another opener", kind: "human" }, caps: ["containers:read", "terminals:spawn"] }, f.root);
    ...
    if (firstCreate?.type !== "create" || !firstCreate.runtime?.privateEnv) throw new Error("native run launch missing");
...
test("live harness inventory owns profile validation and loses execution on disable", async () => {
  const f = await fixture();
  try {
    const inventory = ListHarnessesResultSchema.parse(result(await f.host.dispatch(f.root, "core.access.listHarnesses", {})));
    expect(inventory.harnesses.map((harness) => harness.id)).toContain("test-harness");
    expect(() => f.auth.registerAgent({ name: "Invalid", purpose: "Bad profile", harness: "test-harness", grant: f.registered.agent.grant, context: { profile: { label: 7 } } }, f.root)).toThrow("harness profile invalid");
    const created = await f.create();
    result(await f.host.dispatch(f.root, "engine.plugins.setEnabled", { id: pluginId, enabled: false }));
    expect((await f.host.dispatch(f.root, "core.access.launchRun", { runId: created.run.id })).ok).toBe(false);
    const disabled = ListHarnessesResultSchema.parse(result(await f.host.dispatch(f.root, "core.access.listHarnesses", {})));
...
test("private launches refuse older transports and owners before disclosure and again at native admission", async () => {
  ...
  try {
    ...
    for (const [transport, owner] of [[31, 35], [32, 34]] as const) {
      f.connect(transport, owner);
      const refused = await f.host.dispatch(f.root, "core.access.launchRun", { runId: unlaunched.run.id });
      expect(refused).toMatchObject({ ok: false, denial: { message: "run_launch_protocol_unsupported" } });
      const socket = await f.open(bound.runtime);
      expect(socket.messages()).toContainEqual(expect.objectContaining({
        type: "error", code: "forbidden", message: "run_launch_protocol_unsupported",
      }));
      expect(f.sent.filter((message) => message.type === "create")).toEqual([]);
    }
