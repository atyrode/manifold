...
describe("installArtifact", () => {
  ...
  test("machine bundle admission pins the member and executable before publishing bytes", async () => {
    ...
    try {
      ...
      const artifact = {
        bundleFile: "worker", sha256: hash, entrySha256: hash, format: "raw" as const,
        entry: ["worker"], maxBytes: 4096, maxExpandedBytes: 4096, maxMembers: 1,
      };
      const manifest: PluginManifest = {
        ...MANIFEST,
        machine: {
      ...
      for (const bytes of [
        ...
        bundleBytes({ ...manifest, machine: { ...manifest.machine!, artifacts: {
        ...
        expect((await refusal(() => installArtifact({
          source, sha256: sha256Hex(bytes), dataDir: drop.dataDir,
      ...
      const result = await installArtifact({
        ...
        sha256: sha256Hex(bytes), dataDir: drop.dataDir,
      });
