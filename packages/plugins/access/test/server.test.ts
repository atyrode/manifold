 ...
 * broader than the minter's, no widening of pad scope, revoking only what you minted — is
 * answered by the identity mechanism on the REAL caller, because that is the seam ADR 0011's
...
interface Recorder {
  readonly ctx: Parameters<typeof accessHandlers.mintToken>[0];
  readonly calls: Call[];
}

const principal: Principal = { id: "p-1", kind: "human", name: "delegate", color: "#2563eb" };
const grant: TokenGrant = { token: "raw-secret", principal, caps: ["pads:read"], padId: null };
...
describe("core.access handlers", () => {
  test("a grant is passed through exactly as the mechanism issued it", async () => {
    ...
    const minted = await accessHandlers.mintToken(host.ctx, {
  ...
  test("the caller's request reaches the mechanism unedited", async () => {
    ...
    const request = {
      ...
      caps: ["scene:write" as const],
      padId: "pad-7",
    };

    await accessHandlers.mintToken(host.ctx, request);

    // No normalizing, no defaulting, no dropping of `padId`: the mechanism decides what a
  ...
  test("a refusal keeps the mechanism's wording, so a 403 body becomes a denial message", async () => {
    ...
    const tooWide = recorder({
      mint: { ok: false, code: "forbidden", message: "cannot mint capability terminal:write" },
    });
    ...
    expect(
      refusal(
        await accessHandlers.mintToken(tooWide.ctx, {
          principalId: "p-2",
          caps: ["terminal:write"],
        }),
      ),
    ).toBe("cannot mint capability terminal:write");
    expect(refusal(await accessHandlers.revokeToken(notMine.ctx, { principalId: "p-3" }))).toBe(
  ...
  test("a missing principal refuses rather than throwing, exactly as the route 404'd", async () => {
    ...
    expect(
      refusal(await accessHandlers.mintToken(host.ctx, { principalId: "ghost", caps: ["*"] })),
  ...
  test("revocation answers a count, and zero is a success", async () => {
    ...
    expect(await accessHandlers.revokeToken(nothing.ctx, { principalId: "p-9" })).toEqual({
      revoked: 0,
    });
    expect(await accessHandlers.revokeToken(three.ctx, { principalId: "p-9" })).toEqual({
      revoked: 3,
    });
  });
  ...
  test("no raw secret is ever formatted into a refusal", async () => {
    const host = recorder({
      mint: { ok: false, code: "forbidden", message: "cannot widen pad scope" },
    });

    const reason = refusal(
      await accessHandlers.mintToken(host.ctx, { principalId: principal.id, caps: ["pads:read"] }),
    );
    ...
    expect(reason).toBe("cannot widen pad scope");
