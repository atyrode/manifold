...
/**
 * The tiled discipline's renderer — `core.compositions`'s whole browser half. A View and a
 * Pad are one container object; this module draws the `layout: "tiled"` half, a recursive flex
 * tree over the scene doc's layout key, while `core.canvas` draws the other.
 *
...
/**
 * THE TILED RENDERER, as the projection registry mounts it: `PadSurfaceProps` and nothing of
 ...
 * comes from {@link usePadRoute}, which is the shell's one published answer to "what is the
 * viewer looking at". A second prop bundle carrying the same facts would be a second answer.
 */
export function TiledPadSurface({
  host,
  padId,
  pads,
  presence,
  ...
  navigate,
}: PadSurfaceProps): ReactElement {
  const route = usePadRoute();
  const { onCreateTerminalChange, originPadId, refreshActivePad: onPadChanged } = route;
  /**
   ...
   */
  const padRecord =
    (route.activePad?.id === padId ? route.activePad : null) ??
    pads.find((candidate) => candidate.id === padId) ??
    null;
  const padName = padRecord?.name ?? null;
  /**
  ...
  /**
   * The machine a tiled terminal is running on, straight off the inventory. The dot's colour
  ...
  const machineFor = useCallback(
    (sessionId: string): MachineSummary | null => {
      const session = client.sessions.get(sessionId);
      if (session === undefined) return null;
      return machines?.find((candidate) => candidate.id === session.machineId) ?? null;
    },
    [client, machines],
  );
   ...
   * is what a leaf's pad surface always is — the algebra merges compositions rather than
   * nesting them, so a composition never becomes a leaf of another one.
   */
  const padLayoutFor = useCallback(
    (embeddedPadId: string): Pad["layout"] =>
      pads.find((candidate) => candidate.id === embeddedPadId)?.layout ?? "canvas",
    [pads],
  );
  ...
  const rename = useCallback(
    ...
    [failed, host.client, onPadChanged, padId],
  );
  ...
  }, [failed, host.client, onPadChanged, padId, shrink]);
  ...
  const createTerminal = useCallback(
    async (machine?: MachineSummary): Promise<void> => {
       ...
       */
      const target = machine ?? terminals?.defaultMachine(padId, machines) ?? null;
      if (target !== null) terminals?.rememberMachine(padId, target.id);
      try {
    ...
    [client, failed, machines, notify, padId, terminals],
  );
  ...
  const lookup = useMemo(
    ...
    /*
      ...
      prevent.
    */
    [client, pads, padId, roster, sceneRevision, soloOccupants],
  );
    ...
    switch (surface.kind) {
          ...
          <div className="tiled-pad-tile">
            ...
            <div className="tiled-pad-tile__body">
              ...
              */}
              <PadSurface
                key={surface.padId}
                layout={padLayoutFor(surface.padId)}
                host={host}
                padId={surface.padId}
                depth={2}
                navigate={navigate}
      ...
      case "panel": {
        ...
        */
        ...
        return <Placeholder name={surface.panelId} state="unknown" />;
      }
      default: {
  ...
  return (
    <div className="tiled-pad-view">
      <NodeTitleBar
        ...
        onClose={removeView}
        closeLabel={`Delete view ${padName ?? padId}`}
      ...
      >
        ...
        */}
        ...
        {/*
          ...
          receipt belong to `core.presence` and reach every mounted surface through the same
          two overlay slots the canvas mounts. Remote cursors and carry ghosts above are this
          renderer's own paint, because only this renderer knows that a tiled room measures in
