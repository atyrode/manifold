 ...
 *
 * Three refs, one concept each:
 *
 *   `TerminalView`       the viewer for one PTY: chrome, controls, focus presence, restart.
 *   `buildTerminalRows`   the janitor projection — which terminals a caller can see and act on,
 *                        with `canKill` computed from the kill door's own rule.
 *   machine choice       per-container, per-device memory of which machine a terminal is born on,
...
export {
  buildTerminalRows,
  type TerminalInventoryInput,
  type TerminalRow,
} from "./terminal-inventory.ts";
export {
  browserMachineStorage,
  chooseDefaultMachine,
  machineMemoryKey,
...
/**
 * The birth policy, as a ref sees it: this device's memory for the container, then the
