 ...
 * element a resizer and a commit path. Everything else on the board arrives from the
...
function buildNodeTypes(
  ...
  // surface's own addressing species at the one place it could bite).
 ...
 * the answer instead ({@link PadSurfaceProps.soloOccupants}); this is the fallback for
...
/** One peer's selection outline, in this surface's own scene coordinates. */
 ...
 * sidebar — come from {@link usePadRoute}, and only while this mount IS the route (depth 1).
 * That is what lets one component be both the routed board and a tile leaf's embedded one.
 */
export function FlowPadView({
  ...
  const [client] = useState(
    () => new SessionClient({ url: sessionUrl(), padId, token: host.token }),
  );
  ...
  */
  ...
  useEffect(() => {
    ...
    /**
     * The SDK emits `roster_changed` for every presence frame, selection payloads included.
     ...
     * `client.roster` directly) plus a colour per principal, and both come off the wire.
  ...
  */
  ...
  /**
   * Remote selection outlines are pure presence, painted into THIS surface's coordinate space
   ...
   * part of its own surface). `rosterRevision` moves on every `roster_changed`, which the SDK
  ...
  const remoteSelections = useMemo<readonly RemoteSelectionRect[]>(() => {
    ...
    return rects;
  }, [client, projected, rosterRevision]);
  ...
  const lookup = useMemo(
    () =>
      createPlacementLookup({
        ...
        */
        roster: host.composition.roster(),
      }),
    /*
      ...
      vocabulary does.
    */
    [client, pads, padId, sceneRevision, soloOccupants, host.composition, projection.revision],
  );
  ...
  const handleNodeDragStop = useCallback(
    (event: MouseEvent | TouchEvent, node: Node): void => {
      ...
      /*
        ...
        Only the ROUTED board can be over the sidebar: an embedded one is inside a tile.
      */
      const unplaced =
        routed &&
        point !== null &&
  ...
  const createTerminal = useCallback(
    async (machine?: MachineSummary): Promise<void> => {
      ...
      const target = machine ?? facet?.defaultMachine(padId, machines) ?? null;
      if (target !== null) facet?.rememberMachine(padId, target.id);
    ...
    [canvasCenter, client, machines, notify, padId, terminals],
  );
  ...
  /**
   * How many sessions the open container is holding, for the shell's index row. It is the
   * room's OWN session table and nothing derived: which sessions a janitor should still OFFER
   * (an exited one with no surface left has no remaining action) is `core.terminals`' policy,
   * and re-deciding it here would be a second answer to a question that already has one.
   */
  const sessionCount = useMemo(() => {
    void sceneRevision;
    return client.sessions.size;
  ...
  /*
    ...
    verb a section can offer. Only the ROUTED board reports: an embedded one is not what "the
  ...
  */
  ...
  }, [createTerminal, route, routed, savedAt, sceneRevision, sessionCount, status]);
    ...
    <div className="flow-pad-view">
      <div
        ...
        onKeyDown={(event) => {
          ...
          if (!modifier && !event.altKey) {
            ...
            // is this surface's, the vocabulary is the composition's.
            if (
              nextTool !== null &&
      ...
      >
        {/*
          ...
          surface, `core.presence` owns what goes in it, and neither package names the other.
          Only the routed board mounts them: an embedded board would stack a second island
          and centre a second time for one ask.
        */}
        {routed ? (
          <>
            <div className="flow-presence">
              <PadOverlayOutlet slot="pad-roster" client={client} padId={padId} host={host} />
            </div>
            <PadOverlayOutlet slot="pad-spotlight" client={client} padId={padId} host={host} />
          </>
        ) : null}
