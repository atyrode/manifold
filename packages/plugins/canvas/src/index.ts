 ...
 *
 * This is the renderer half of what used to be `core.shell.pad-view`: the React Flow
 * projection boundary, the node-type map, the viewport and its per-pad camera memory, the
 * tool strip, portal widgets, and the gesture wiring that turns a pointer into document
 ...
 * to somebody else's door and is dispatched by name: `core.terminals.rename` from a widget
 * titlebar, `core.views` for a container's life, `core.layout.place` for every placement. A
 * canvas action would be a second door onto one of those (invariant 14).
 *
 * `scene:write` is declared for the same reason `core.draw` and `core.notes` declare it: it is
 ...
 * every mode the surface offers, not just the ones other plugins added. `core.draw`'s tool
 * arrives through the same registry, which is why the strip has no literal naming it.
 *
 * No `panels`. A pad renderer is reached by LAYOUT, through the projection's pad-surface
 * registry (`padSurfaces` in the browser half): the routed shell asks for the surface of the
 * container it is showing, and a composition's tile leaf asks for exactly the same thing when
 * it embeds a board. One door onto "project a pad of this discipline", used identically by
 ...
 * `core.draw`, and reach this surface through the element registry like any other plugin's.
 */
export const canvasManifest: PluginManifest = {
  ...
  title: "Canvas",
  description:
    "The infinite canvas: the React Flow renderer, portal widgets, the tool strip and per-pad camera memory.",
