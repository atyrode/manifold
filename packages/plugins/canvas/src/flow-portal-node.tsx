...
function usePadName(host: HostServices, padId: string): string | null {
      ...
      */}
      <TerminalSurface
        ...
        machine={
          machineId === undefined
            ? null
            : (pad.machines?.find((candidate) => candidate.id === machineId) ?? null)
        }
...
function PortalPadTile({ padId }: { readonly padId: string }): React.ReactElement {
  const pad = useFlowPad();
  const name = usePadName(pad.host, padId);
  return (
...
function PortalNodeImpl({ id, data }: NodeProps): React.ReactElement {
  ...
  const name = usePadName(pad.host, containerId);
