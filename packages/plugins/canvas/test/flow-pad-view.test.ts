...
import { nodeTypesFor } from "../src/flow-pad-view.tsx";
import { PortalNode } from "../src/flow-portal-node.tsx";
 ...
 * every roster change and hands this surface a FRESH element registry each time, so the map
...
describe("nodeTypesFor", () => {
  ...
  test("this surface's own species is not overridable by a manifest that claims it", () => {
    const map = nodeTypesFor(
    ...
    // stranger could otherwise shadow the surface's OWN species — `portal` is applied last
