...
"change nothing" is written `Status: accepted` with a `Ratified:` line saying it is an evaluation
verdict and never enters this table — it is kept for the next reader, not for the operator's
pen. The first such record is
...
| [`0018-trace-ledger.md`](docs/decisions/0018-trace-ledger.md)         | 2026-09-07, A6 wording (#396)                                | The constitutional addition in [#93](https://github.com/atyrode/manifold/issues/93) / [PR #100](https://github.com/atyrode/manifold/pull/100) is confirmed by [#396](https://github.com/atyrode/manifold/issues/396), discharging the wording review. Every registered door is traced, including refusals; A6's existing exemptions and T1–T5 enforcement remain unchanged.                                               |
| [`0019-identity-posture.md`](docs/decisions/0019-identity-posture.md) | 2026-09-01; lifetime policy amended by operator request #326 | The recovery owner key remains non-expiring; ordinary humans expire after fourteen days and ordinary agents after one hour. Machine enrollment and terminal-lifecycle credentials are explicit internal exceptions, not a blanket agent exemption. Schema 24 bounds legacy ordinary credentials after a migration grace period. The normative policy is `docs/CONTRACTS.md`; optional OIDC evaluation is tracked in #324. |
...
- **Refusals are named classes, not one boolean.** `essential` (the four rail non-negotiables
  ...
  toggle; children meet `dependency_disabled` (ADR 0023 §9, ratified 2026-09-05) — the dependency
  classes say it, and no class was added. The STRUCTURAL problems assembly throws are a different
  set from these classes, and it grew once: duplicates, squats, cycles, self-dependency, and
  `orphan_child` — a three-segment id with no `required` edge on its parent (ADR 0023 §2;
...
```json
{
  "lexicon": [
    ...
    {
      "term": "stroke",
      "means": "one freehand ink record: `core.canvas.draw`'s element kind, the sampling distance a gesture retains points at, and the width it is painted with. A stroke is not a POLYLINE — the ink is the domain noun, the coordinates are the geometry — and only a plugin may say the word",
      "banned": [],
      "allow": []
    },
    {
      "term": "polyline",
      "means": "a flat `[x0, y0, x1, y1, …]` coordinate sequence and the geometry over it: extents, SVG path data, a viewBox, a rebase onto an origin (`packages/plugin/src/polyline.ts`). The neutral half of what used to be two private copies of stroke math (issue #117) — it names no plugin and no element kind, which is why the ENGINE may hold it while `stroke` stays `core.canvas.draw`'s word",
      "banned": [],
      "allow": []
    },
    {
...
```json
{
  "cssFamilies": [
    ...
    {
      "family": "draw",
      "owner": "packages/plugins/canvas/draw/src/styles.css",
...
```json
{
  "deviceLocal": [
    ...
    {
      "key": "manifold.previewNonce",
      "why": "one browser's public nonce for the production-to-preview identity handoff, retained in sessionStorage only across the two top-level navigations that complete that handoff and removed on success; the preview server separately binds it to undisclosed host-only cookie state before minting"
    },
    {
...
```json
{
  "gateContracts": [
    ...
    },
    {
      ...
      "renderer": "packages/plugins/plugin-manager/src/web.tsx",
      "why": "R3 expands the canvas family through its own chevron before pressing the child core.canvas.draw toggle; aria-expanded prevents a repeated wait from folding the family again"
    },
    {
...
| R5    | Presence and spotlight: a picked tool is visible to an SDK peer as `vantage.tool` within 2s; a connection's mounted path paints its principal in the shared titlebar, explicit null retracts it, and hot disable/re-enable removes/restores attendance without reload or republish; with two shared rooms, `core.presence.focus` selects an eligible URI-named room before the caller's most recently joined eligible room, and uses that recent room for unqualified addresses; the target's viewport centers through the debug probe and a container-scoped token invoking focus is `forbidden`.                                                                                                                                                                                                                                                                                                                                                  |
...
| Foundation law (litmus, pillars)                            | S2, S6, S7, S9, S13                                                                                                                                                                                      |
