...
| [`0023-plugin-topology.md`](docs/decisions/0023-plugin-topology.md)   | waiting — proposed 2026-09-05             | A three-segment plugin id is a claim of home that assembly refuses without a `required` edge on the parent (`orphan_child`); ids cap at three segments; a child is a directory inside its parent's package behind subpath exports (S18 import direction); `core.draw` → `core.canvas.draw` in a later rename wave; `core.notes` splits into `core.text` + `core.canvas.note` |
