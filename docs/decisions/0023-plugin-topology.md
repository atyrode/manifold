...
Date: 2026-09-05
Status: accepted
Ratified: by the operator, 2026-09-05, in full including §6 (#252). The rename wave (§10) and the S18 check are obligations, not acceptance.

The living spec carries this decision: `docs/CONTRACTS.md` §Plugins, actions, and the workspace
layout (the claim/proof rule, `orphan_child`, the depth cap, what nesting does not do, families
under a toggle), `docs/PLUGINS.md` §1 and §2 (the layout, the import direction, the litmus in
authoring terms, the id rule) and `REGISTRY.md` §Gates (S18). Where this record and the spec
disagree, the spec wins and this record is stale. Line references below are to `main` at
7921f5b, the tree the decision was taken against.
...
Spec: `docs/CONTRACTS.md` §Plugins, actions, and the workspace layout.
...
Spec: `docs/CONTRACTS.md` §Plugins, actions, and the workspace layout (assembly's structural
truths). Enforcement is #261.
...
Spec: `docs/PLUGINS.md` §2. The manifest (the id rule); `docs/CONTRACTS.md` §Plugins, actions,
and the workspace layout. The schema change is #261.

`publisher.product.part` or `core.<seat>.part`; a fourth segment is a design smell and is refused
by the schema: `PLUGIN_ID_PATTERN = /^[a-z][a-z0-9-]*(\.[a-z][a-z0-9-]*){1,2}$/`
...
Spec: `docs/PLUGINS.md` §1. Anatomy ("A part lives inside its parent's package").
...
Spec: `docs/PLUGINS.md` §1. Anatomy ("A part lives inside its parent's package");
`REGISTRY.md` §Gates, row S18 (reserved; lands with #261).
...
files are not walked at all. Hence **S18 — import direction follows the plugin tree**, on PATHS:
...
edge stays S2's offence. RED names file and line, as S2 does. S18 is reserved in the check
inventory and lands with #261.
...
Spec: `docs/CONTRACTS.md` §Plugins, actions, and the workspace layout (the family rule the two
seats will satisfy); the seats themselves are #263.

A document is first-class: `core.text`, a standalone peer — own panel and route, own
...
Spec: `docs/PLUGINS.md` §1. Anatomy (the litmus these verdicts apply). The renames are #262 and
#263.

Every `packages/plugins/*/src/index.ts` manifest read at `main` (contributions, edges, element
kinds); `packages/plugins/` prefixes elided. The verdicts are ratified; the migrations they name
are the rename wave (§10).
...
Spec: `docs/CONTRACTS.md` §Plugins, actions, and the workspace layout; `REGISTRY.md` §Disable
semantics (D4′).
...
Spec: `docs/CONTRACTS.md` §Plugins, actions, and the workspace layout (the manager's order);
`docs/PLUGINS.md` §7. Assembly rules, "Installing a plugin" (the manager's families). Shipped in
#243.
...
Spec: none — a scoping rule for #262 and #263, not a contract.
...
Resolved by this ratification, as follows.

- **T1 — invariant 12** ("a feature lands as a package under `packages/plugins/*`",
  `AGENTS.md:159-166`): accepted as unchanged; a child lands under a package there. S5's package
  walk (`verify-axioms.ts:493`) is unchanged too, and its "every composed plugin maps back to a
  package" (`:886-890`) accepts a child mapping back to its parent's when the first child lands
  (#261).
- **T2 — REGISTRY §Disable semantics D4′** (`REGISTRY.md:502-557`): the per-kind table is
  untouched. The refusals rule gains the one sentence — a family control is the parent's toggle;
  children meet `dependency_disabled` — in this ratification.
- **T3 — ADR 0013 §5 rule 6** says the structural set "is closed" (`:343-345`). The set grows by
  `orphan_child` in `docs/CONTRACTS.md` and `REGISTRY.md` §Disable semantics, and 0013's
  `Ratified:` line carries the note; its body is history and is not rewritten. The regex in §3
  stays out of 0013's dependency section, as 0013 intended (`:350-355`).
- **T4 — AXIOMS A1** (`AXIOMS.md:23-39`): nesting is data on the roster, not mechanism in the
  ...
  the two reserved prefixes. Said out loud here: one check, structural, fatal, in the D5 style, and
  that is the whole of it. No engine branch reads a third segment for any other purpose.
- **T5 — S2** (`verify-axioms.ts:706-731`) is a ratified check that S18 amends, not extends: the
  own-name rule at `:710` becomes a path rule when S18 lands (#261); until then S2 stands as
  written.

## Work this obliges

1. [#261](https://github.com/atyrode/manifold/issues/261) — S18 import-direction check, the
   `orphan_child` assembly problem and §3's depth-3 id pattern.
2. [#262](https://github.com/atyrode/manifold/issues/262) — the rename wave: the `contract`
   subpath convention, `core.draw` → `core.canvas.draw` under `packages/plugins/canvas/draw/` as
   the first nested child (§10), and `core.arrange`'s owed edge on `core.space`.
3. [#263](https://github.com/atyrode/manifold/issues/263) — `core.text` extracted from
   `core.notes`, `core.canvas.note`, and the editor dependency decision (§6).
4. [#264](https://github.com/atyrode/manifold/issues/264) — atyrode/code and atyrode/babel adopt
   the layout in their own repositories (tracking pointer).
