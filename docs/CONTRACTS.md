...
`MANIFOLD_IDENTITY_AUTHORITY=<production-origin>` sends an unauthenticated browser through that
origin's `/auth/preview` shell route. Before leaving, the preview creates a 256-bit nonce plus a
separate opaque state id, returns only the nonce to browser code, and binds the state id to that
browser in a two-minute, host-only, HttpOnly callback cookie. Production reads its own origin-local
browser identity and issues a 60-second Ed25519 assertion for one exact audience and that nonce.
The assertion is submitted to `/auth/preview/callback` in a POST body, verified, and staged under
a fresh opaque ticket delivered only to the receiving browser in a second host-only HttpOnly
cookie. Same-origin finalize requires both that ticket and the initiating state cookie, with
matching signed nonce, before minting. An attacker who starts a flow cannot complete another
browser's callback. No production owner key or reusable bearer enters preview code, a URL, a
cookie, or deployment data. Both short-lived cookies are host-only and cleared at completion.

Production enables issuance with `MANIFOLD_PREVIEW_DOMAIN=<domain>`. Valid audiences are exactly
`https://preview.<domain>` and `https://<positive-integer>.<domain>` on their default HTTPS port;
`.localhost` HTTP is admitted only so the same contract can run in local verification. The preview
router is the lifecycle registry: deployment makes an audience reachable and teardown removes it,
with no per-preview secret or manual production row.

Any valid production owner or workspace-wide local human credential may issue. Container-scoped
credentials are refused because their production container has no honest whole-preview
equivalent. An unrestricted root remains `*`; a root affected by administered denies and every
...
identity ids, audience/issuer and expiry, never credential or assertion bytes. The handshake is
the identity boundary before a local actor exists, not an action-plane mutation; the accepted
...
| ------------------------------------ | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET /healthz                         | none                                  | → `{ ok, version, build, channel, protocolVersion }` — `version` the last release tag without `v`; `build` `= version` at that tag, `<version>+<distance>.g<sha7>` past it (`.dirty` for uncommitted changes); `channel` `release` when build equals version, else `development` (`scripts/build-identity.ts`; `MANIFOLD_VERSION`/`MANIFOLD_BUILD`/`MANIFOLD_CHANNEL` override per field). `build` and `channel` are additive-optional on the wire; a current server always sends them |
...
| GET /api/identity/preview-config     | none                                  | → `{ authority }`; null outside a configured preview                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| GET /api/identity/preview-key        | none                                  | → production's Ed25519 public verification key                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| POST /api/identity/preview-assertion | valid unscoped production bearer      | `{ audience, nonce }` → one signed 60-second assertion with effective authority                                                                                                                                                                                                                                                                                                                                                                                                        |
| POST /api/identity/preview-start     | none                                  | → `{ nonce }`; sets an undisclosed browser-state cookie                                                                                                                                                                                                                                                                                                                                                                                                                                |
| POST /auth/preview/callback          | signed assertion form                 | verifies the assertion and stages it under a ticket delivered only in a host-only callback cookie                                                                                                                                                                                                                                                                                                                                                                                      |
| GET /auth/preview/finalize           | initiating state and callback cookies | consumes both proofs, stores the preview-local browser identity on this origin, and redirects to `/`                                                                                                                                                                                                                                                                                                                                                                                   |
|                                      |                                       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |     |     |     | parent of 29a50e0 (plugin: format workspace enum settings (#158)) |
| Method+Path                          | Auth cap                              | Req → Res                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------------          | ---------------------                 | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET /healthz                         | none                                  | → `{ ok, version, build, channel, protocolVersion }` — `version` the last release tag without `v`; `build` `= version` at that tag, `<version>+<distance>.g<sha7>` past it (`.dirty` for uncommitted changes); `channel` `release` when build equals version, else `development` (`scripts/build-identity.ts`; `MANIFOLD_VERSION`/`MANIFOLD_BUILD`/`MANIFOLD_CHANNEL` override per field). `build` and `channel` are additive-optional on the wire; a current server always sends them |
| GET /api/protocol                    | none                                  | → generated JSON-Schema of all wire messages, plus the published placement, plugin/action, event and grant vocabularies                                                                                                                                                                                                                                                                                                                                                                |
...
> > > > > > > 29a50e0 (plugin: format workspace enum settings (#158))
...
- **Web app identity**: `packages/web/public/app.webmanifest` and the two public SVGs are
  ...
  The bundle also carries the instance's three-word identity (`version`, `build`, `channel`; §HTTP
  `/healthz`) compiled in by the same derivation, so the sidebar's rev line prints `v<build>` —
  `development · v<build>` off a release — and a `development` build appends ` · development` to
