...
release committer (`atyrode`) are exempt only from issue lifecycle checks. Before tagging,
...
version is mandatory: resume discovers and reuses its retained commit, existing release branch,
...
The shared `scripts/release-provenance.ts` policy verifies a prepared candidate's canonical
delta and latest full-source CI before publishing its branch or requesting merge. Before
tagging and again in the tag workflow, the selected SHA must identify a dedicated,
...
`release/vX.Y.Z` PR must have the same tree, successful PR CI and its current required checks.
