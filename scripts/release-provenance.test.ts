...
// Only the three remote reads the legacy rollback pin performs are answered; any attested
// release read fails, so a legacy tag can pass only through the pinned recovery route.
const LEGACY_FIXTURE = `#!/usr/bin/env bun
const fs = require("node:fs");
const state = JSON.parse(fs.readFileSync(process.env.LEGACY_FIXTURE_STATE, "utf8"));
const args = process.argv.slice(2);
const emit = (value) => console.log(JSON.stringify(value));
const url = args[0] === "api" ? new URL("https://fixture.invalid/" + args[1]) : null;
const base = "/repos/" + state.repository;
if (url && url.pathname.startsWith(base + "/git/ref/tags/")) {
  const tag = url.pathname.slice((base + "/git/ref/tags/").length);
  emit({ ref: "refs/tags/" + tag, object: { sha: state.tagSha, type: "commit" } });
} else if (url && url.pathname === base + "/actions/workflows/ci.yml/runs") {
  const sha = url.searchParams.get("head_sha");
  emit({ workflow_runs: sha !== state.tagSha ? [] : [{
    id: 2001, run_attempt: 1, head_sha: sha, head_branch: "main", event: "push",
    path: ".github/workflows/ci.yml", status: "completed", conclusion: state.ci,
    head_repository: { full_name: state.repository },
  }] });
} else if (url && url.pathname === base + "/actions/runs/2001/attempts/1/jobs") {
  emit({ jobs: [{ name: "gate", status: "completed", conclusion: state.ci }] });
} else {
  console.error("Legacy fixture has no attested release: " + args.join(" "));
  process.exit(1);
}
`;

test("the pinned legacy release is admitted only as a rollback base at its exact green commit", async () => {
  const root = mkdtempSync(join(tmpdir(), "manifold-legacy-recovery-"));
  roots.push(root);
  const bin = join(root, "bin");
  mkdirSync(bin);
  writeFileSync(join(bin, "gh"), LEGACY_FIXTURE, { mode: 0o700 });
  const stateFile = join(root, "state.json");
  const module = new URL("./release-provenance.ts", import.meta.url).pathname;
  const pinned = "5461d479503a5718dcc0f224dfd6d93b35060c6d";
  const green = { repository: "atyrode/manifold", tagSha: pinned, ci: "success" };
  const invoke = async (mode: "promotion" | "recovery", tag: string, state: typeof green) => {
    writeFileSync(stateFile, JSON.stringify(state));
    const child = Bun.spawn([process.execPath, module, mode, tag], {
      cwd: root,
      env: {
        PATH: `${bin}:${process.env.PATH}`,
        GITHUB_REPOSITORY: state.repository,
        LEGACY_FIXTURE_STATE: stateFile,
      },
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  const admitted = await invoke("recovery", "v0.14.0", green);
  expect(admitted.code, admitted.err).toBe(0);
  expect(JSON.parse(admitted.out)).toEqual({
  ...
  // The same remote state never makes the legacy release a promotion candidate.
  expect((await invoke("promotion", "v0.14.0", green)).code).not.toBe(0);
  for (const refused of [
    { ...green, tagSha: "f".repeat(40) },
    { ...green, ci: "failure" },
    { ...green, repository: "fork/manifold" },
  ])
    expect((await invoke("recovery", "v0.14.0", refused)).code).not.toBe(0);
  expect((await invoke("recovery", "v0.13.0", green)).code).not.toBe(0);
}, 20_000);
