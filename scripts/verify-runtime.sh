...
"$timeout" --kill-after=5s 60s "$env_bin" -i PATH="$path" HOME="$root/home" TMPDIR="$root/tmp" \
  "$cc" -static -O2 -Wall -Wextra "$repo/packages/agent/test/fixtures/job-instance-service.c" -o "$root/instance-service"
static_elf "$root/instance-service"
...
proof_env=(PATH="$path" HOME="$root/home" TMPDIR="$root/tmp" LANG=C.UTF-8
  ...
  MANIFOLD_TEST_INSTANCE_SERVICE="$root/instance-service"
