    ...
    if mode == "install":
        ...
        declaration = {
            ...
            "operations": {operation_id: {
                "argv": [], "input": {}, "runtimeTools": ["busybox"], "locations": [],
        ...
        for capability in ["jobs:read", "machines:run"]:
            action("engine.jobs.consent", {
                **installation, "cap": capability, "enabled": True,
                "node": "manifold://machine/" + machine["id"] + "/operation/" + operation_id,
            })
    elif mode == "execute":
