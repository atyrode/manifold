...
try {
  ...
  for (const pid of pids) {
    ...
    if (pid === "1") {
      if (args.length !== 2 || !bun(args[0]) || args[1] !== "packages/server/src/main.ts" ||
          executable !== "/usr/local/bin/bun" || readlinkSync("/proc/1/cwd") !== "/app") throw new Error();
      server = true;
      continue;
    }
    ...
    if (args.length === 3 && bun(args[0]) && args[1] === "-e" && args[2] === healthSource &&
        executable === "/usr/local/bin/bun") continue;
    if (args.length === 3 && args[0] === "/bin/sh" && args[1] === "-c" && args[2] === healthCommand &&
        executable === "/usr/bin/dash") continue;
    throw new Error();
  }
  // starttime is stable across the probe, unlike CPU counters in /proc/1/stat.
  const starttime = (stat: string): string | undefined => stat.slice(stat.lastIndexOf(")") + 2).split(" ")[19];
  if (!server || !starttime(before) || starttime(before) !== starttime(readFileSync("/proc/1/stat", "utf8")))
    throw new Error();
