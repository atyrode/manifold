...
if [[ -f "$PREVIEW_HOME/env" ]]; then
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    key=${line%%=*}; value=${line#*=}
    case "$key" in
      PREVIEW_DOMAIN|PREVIEW_SEED|PREVIEW_DEV_CHECKOUT|PREVIEW_DEV_URL|PREVIEW_DEV_PORT|PREVIEW_PORT_RANGE|PREVIEW_ROUTER_PORT|MANIFOLD_DEV_SERVICE_OWNER_MACHINE_ID|MANIFOLD_DEV_SPAWN_AGENT|MANIFOLD_DEV_SPOKE_UNIT|MANIFOLD_DEV_SPOKE_BINARY|MANIFOLD_DEV_SPOKE_ENV) export "$key=$value" ;;
