...
const artifact = z.object({ path: artifactPath, realPath: artifactPath, sha256: z.string().regex(/^[a-f0-9]{64}$/) });
const runtime = z.object({ tini: artifact, bash: artifact, nixDaemon: artifact, bun: artifact, sleep: artifact, entrypoint: artifact });
const reference = z.object({
  pid: z.number().int().positive(), startTime: z.number().int().positive(),
  role: z.enum(["init", "supervisor", "daemon", "server", "owner", "transport"]),
  executable: artifact.shape.path, sha256: artifact.shape.sha256,
  parent: z.number().int().nonnegative(), uid: z.union([z.literal(0), z.literal(1000)]),
  group: z.number().int().positive(), session: z.number().int().positive(),
  argv: z.array(z.string()).optional(),
});
const requestSchema = z.discriminatedUnion("mode", [
  z.object({ mode: z.literal("source"), sources: z.record(z.string(), artifact.shape.sha256), runtime }),
  z.object({ mode: z.literal("inventory"), references: z.array(reference).min(4).max(6), runtime, supervisorPid: z.number().int().positive() }),
...
try {
  ...
  if (request.mode === "source") {
    ...
    for (const directory of ["/app", "/home/developer"])
      check(!readdirSync(directory).some((name) => name.startsWith(".env") || name === ".bunfig.toml"));
    check(!existsSync("/bunfig.toml") && !existsSync("/.bunfig.toml"));
    for (const [path, sha256] of Object.entries(request.sources)) {
      check(path.startsWith("/app/") && !path.split("/").includes("..") && /^[a-f0-9]{64}$/.test(String(sha256)));
    ...
    check(!readdirSync("/sys/fs/cgroup").some((name) => lstatSync(`/sys/fs/cgroup/${name}`).isDirectory()));
    ...
    for (const name of pids) {
      ...
      if (reference) {
        check(state[19] === String(reference.startTime) && !["Z", "X"].includes(state[0]!) &&
          Number(state[1]) === reference.parent && Number(state[2]) === reference.group &&
          Number(state[3]) === reference.session);
        const uid = readFileSync(`/proc/${pid}/status`, "utf8").split("\n").find((line) => line.startsWith("Uid:"))?.trim().split(/\s+/).slice(1);
        check(uid?.length === 4 && uid.every((value) => value === String(reference.uid)));
        if (reference.uid === 0) {
          check(JSON.stringify(args) === JSON.stringify(reference.argv) &&
      ...
      const sleeper = Number(state[1]) === request.supervisorPid &&
        JSON.stringify(args) === JSON.stringify([request.runtime.sleep.path, "0.1"]) &&
        executable === request.runtime.sleep.realPath;
      const health = JSON.stringify(args) === JSON.stringify(["bun", "-e", healthSource]) &&
        executable === request.runtime.bun.realPath;
      check(sleeper || health);
      check(hash(`/proc/${pid}/exe`) === (sleeper ? request.runtime.sleep.sha256 : request.runtime.bun.sha256));
    }
    check(found.size === expected.size);
  } else {
    const references = request.references;
    check(process.getuid?.() === 1000 && Array.isArray(references) && references.length >= 1 && references.length <= 3);
    ...
    try {
      for (const reference of references) {
        check(Number.isSafeInteger(reference.pid) && reference.pid >= 2 &&
          Number.isSafeInteger(reference.startTime) && reference.startTime > 0 &&
          typeof reference.executable === "string" && reference.executable.startsWith("/nix/store/") &&
          /^[a-f0-9]{64}$/.test(reference.sha256) && ["owner", "transport", "server"].includes(reference.role));
        const fd = libc.symbols.pidfd_open(reference.pid, 0);
        check(fd >= 0);
        handles.push(fd);
        const state = fields(reference.pid);
        check(state[19] === String(reference.startTime) && !["Z", "X"].includes(state[0]!));
        check(Number(state[1]) === reference.parent && Number(state[2]) === reference.group &&
          Number(state[3]) === reference.session && readFileSync(`/proc/${reference.pid}/cgroup`, "utf8").trim() === "0::/");
        if (reference.role === "owner")
          check(lstatSync("/data/terminal-host").isDirectory() && lstatSync("/data/terminal-host/host.sock").isSocket());
        const args = argv(reference.pid);
        const suffix = reference.role === "server" ? ["packages/server/src/main.ts"] :
          ["packages/agent/src/main.ts", ...(reference.role === "owner" ? ["--terminal-host"] : [])];
        check(["bun", reference.executable].includes(args[0]!) &&
